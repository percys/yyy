...
public class InternalYgDaoImpl extends HibernateDaoSupport implements InternalYgDao {

	public Long getCommodityTotal(QueryInternalNbjs toto) {
		String hql = "select count(*) from TtotdzrptShnbjs where 1=1 and orgcode='"+toto.getOrgcode()+"'";
	...
	public List<TtotdzrptShnbjs> doSplitPage(final int page, final int rows,
			final QueryInternalNbjs toto) {
		List list = getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(Session session)
					throws HibernateException, SQLException {
				String hql = "from TtotdzrptShnbjs where 1=1 and orgcode='"+toto.getOrgcode()+"'";
				List<String> values = new ArrayList<String>();
				if (toto != null) {
					hql = jointHQL(hql, toto, values);
				}
