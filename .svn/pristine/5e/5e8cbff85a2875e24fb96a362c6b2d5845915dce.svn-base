...
public class TtotdzrptDaoImpl extends HibernateDaoSupport implements TtotdzrptDao {

	public Long getCommodityTotal(QueryInternalNbjs toto) {
		String hql = "select count(*) from Ttotdzrpt where 1=1";
	...
	public List<Ttotdzrpt> doSplitPage(final int page,final int rows, final QueryInternalNbjs toto) {
		List list = getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(Session session)
				...
				String hql = "from Ttotdzrpt where 1=1";
				List<String> values = new ArrayList<String>();
				if (toto != null) {
					hql = jointHQL(hql, toto, values);
				}
