...
public class ExternalDaoImpl extends HibernateDaoSupport implements ExternalDao {

	public Long getCommodityTotal(QueryInternalNbjs toto) {
		String hql = "select count(*) from TtotdzrptShwbjs where 1=1";
	...
	public List<TtotdzrptShwbjs> doSplitPage(final int page, final int rows,
		...
		List list = getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(Session session)
				...
				String hql = "from TtotdzrptShwbjs where 1=1";
				List<String> values = new ArrayList<String>();
				if (toto != null) {
					hql = jointHQL(hql, toto, values);
				}
