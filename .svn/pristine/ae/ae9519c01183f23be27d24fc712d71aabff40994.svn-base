...
function csv2(){
	...
	var grid = $('#webWBjmDatagrid');  
	var options = grid.datagrid('getPager').data("pagination").options;  
    var url = 'ttotdzrptwb_csv.action?page='+options.pageNumber+'&rows='+options.total+'&orgcode='+orgcode+'&begindate='+begindate+'&enddate='+enddate;  
    window.location.href = url; 
};
function onSelectRow(rowIndex, rowData){
	var dg=$('#webWBjmDatagrid').datagrid('getSelections');
	if(dg.length!=1){
...
<div id="Wbjmtoolbar">
	<form id="WBjmWebForm">
		...
		组织编码：<input id="orgcode" type="text" name="ttotdzrpt.orgcode" style="width: 150px; ">
		<a href="javascript:" class="easyui-linkbutton" plain="false" iconCls="icon-search" onclick="searchForm('#webWBjmDatagrid','#WBjmWebForm')">查询</a>
		<a href="javascript:" class="easyui-linkbutton" plain="false" iconCls="icon-search" onclick="formClean('#WBjmWebForm')">清空</a>
		<a href="javascript:" class="easyui-linkbutton" plain="true" iconCls="icon-print" onclick="csv2()">导出</a>
	</form>
</div>
<table id="webWBjmDatagrid" class="easyui-datagrid" style="height: 400px; width: 100px;"
data-options="title:'账单详细列表',url:'ttotdzrptwb_getAll.action',border:false,fit:true,fitColumns:false,columns:[[
...
]],toolbar: '#Wbjmtoolbar',pagination:true,nowrap:false,onSelect:onSelectRow,onUnselect:onSelectRow               
">                                                                                                                      
</table>