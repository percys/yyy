...
public class TtotdzrptNbYgDaoImpl extends HibernateDaoSupport implements TtotdzrptNbYgDao {

	public Long getCommodityTotal(QueryInternalNbjs toto) {
		// TODO Auto-generated method stub
				String hql = "select count(*) from TtotdzrptNb where 1=1 and orgcode='"+toto.getOrgcode()+"'";
	...
	public List<TtotdzrptNb> doSplitPage(final int page, final int rows,
		...
		List list = getHibernateTemplate().executeFind(new HibernateCallback() {
			public Object doInHibernate(Session session)
				...
				String hql = "from TtotdzrptNb where 1=1 and orgcode='"+toto.getOrgcode()+"'";
				List<String> values = new ArrayList<String>();
				if (toto != null) {
					hql = jointHQL(hql, toto, values);
				}
